...
# Add ContextualDiff composable before the last closing brace
contextual_diff = '''
@Composable
private fun ContextualDiff(originalContent: String, patchedContent: String) {
    ...
    // Find inserted line indices in patched content
    val insertedIndices = patchedLines.mapIndexedNotNull { i, line ->
        if (line !in originalSet) i else null
    }.toSet()

    // Build display ranges: show context around each inserted line
            ...
            toShow.sorted().forEach { lineIdx ->
                ...
                Row {
                    ...
                    Text(
                        ...
                        lineHeight = 14.sp,
                        modifier = if (isInserted) Modifier.background(Color(0xFF0A1A0A)) else Modifier
                    )
                }
                lastShown = lineIdx
            }
...
print("PatchPreviewScreen.kt updated")
EOF
